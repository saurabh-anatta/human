...
class PredictiveSearchComponent extends Component {
  ...
  connectedCallback() {
    ...
    if (dialog) {
      ...
      dialog.addEventListener(DialogOpenEvent.eventName, this.#handleSearchIconOpen, { signal });
      ...
      document.addEventListener(
        'click',
        (event) => {
          if (event.target instanceof Element && event.target.nodeName === 'HTML') {
  ...
  #handleDialogClose = () => {
    ...
    document.querySelectorAll('search-button').forEach((btn) => {
      btn.classList.remove('search-is-open');
    });
  };

  /**
   * Swaps the header search icon to the close icon when the modal opens.
   */
  #handleSearchIconOpen = () => {
    document.querySelectorAll('search-button').forEach((btn) => {
      btn.classList.add('search-is-open');
    });
  };

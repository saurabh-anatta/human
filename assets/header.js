...
class HeaderComponent extends Component {
  ...
  /**
   * Tags the search modal when the header search icon is clicked so CSS can
   * apply the slide-in animation instead of the default fade used by the menu drawer.
   * Registered in capture phase so the class is set before the dialog handler fires.
   * @param {MouseEvent} event
   */
  #onSearchIconClick = (event) => {
    if (!(event.target instanceof Element)) return;
    if (!event.target.closest('.search-action button')) return;
    document.getElementById('search-modal')?.classList.add('search-modal--from-header');
  };

  /** Removes the header-open tag so the next open starts with a clean state. */
  #onSearchModalClose = () => {
    document.getElementById('search-modal')?.classList.remove('search-modal--from-header');
  };
  ...
  connectedCallback() {
    ...
    document.addEventListener('click', this.#onSearchIconClick, true);
    document.getElementById('search-modal')?.addEventListener('dialog:close', this.#onSearchModalClose);
  ...
  disconnectedCallback() {
    ...
    document.removeEventListener('click', this.#onSearchIconClick, true);
    document.getElementById('search-modal')?.removeEventListener('dialog:close', this.#onSearchModalClose);
